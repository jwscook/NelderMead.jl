...
@testset "NelderMead tests" begin
  @testset "ND Rosenbrock" begin
    ...
    @testset "Single solve - initial position and simplex size" begin
      for N ∈ 1:10
        solution = NelderMead.solve(x->objective(x, N), zeros(N), ones(N),
                                      stopval=stopval, maxiters=100_000)
        test_solution(solution, N)
      end
    end
    @testset "Single solve - vertex locations" begin
      for N ∈ 1:10
        solution = NelderMead.solve(x->objective(x, N), [rand(N) for i ∈ 1:N+1],
                                    stopval=stopval, maxiters=100_000)
        test_solution(solution, N)
      end
    end
    @testset "Grid solve" begin
      for N ∈ 1:3
        ...
        solutions = NelderMead.solve(x->objective(x, N), zeros(N), ones(N), gridsize,
                                    stopval=stopval, maxiters=100_000)
    ...
    @testset "Errors are caught" begin
      N = 2
      @test_throws ArgumentError NelderMead.solve(x->objective(x, N), [rand(1),
                                                  rand(2), rand(3)])
      @test_throws ArgumentError NelderMead.solve(x->objective(x, N), rand(2),
                                                  rand(3))
      @test_throws ArgumentError NelderMead.solve(x->objective(x, N), [0.0],
                                                  [0.0], [1])
      @test_throws ArgumentError NelderMead.solve(x->objective(x, N), rand(1),
                                                  rand(2), [1, 2, 3])
      @test_throws ArgumentError NelderMead.solve(x->objective(x, N), rand(2),
    ...
    @testset "Stretched grid causes endless loop" begin
      ...
      solution = NelderMead.solve(stretchedobjective, zeros(N), ones(N),
    ...
    @testset "Nan causing endless loop" begin
      ...
      solution = NelderMead.solve(nanobjective, zeros(N), ones(N) / 2,
                                  maxiters=1000, xtol_rel=2eps())
      _, _, returncode, numiters = solution
      @test returncode == :ENDLESS_LOOP
    end
